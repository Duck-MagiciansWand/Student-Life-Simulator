...
private:
    ...
    int loneliness = 30;      // 0-100, 越高越孤独
    int emotionalStability = 70; // 0-100, 越高越稳定
    std::vector<std::string> memory; // 情感记忆
...
private:
    ...
    int intimacy = 0;        // 0-100
    int affection = 0;       // 0-100 好感度
...
public:
        ...
        discoveries = {
            {
                "secret_garden",
                "🌸 秘密花园",
                "在图书馆后面发现了一个隐藏的小花园，安静而美丽。",
                "图书馆后方",
                [](Stats& stats, EmotionalState& emotion) {
                    emotion.modifyLoneliness(-15);
                    emotion.setEmotion(Emotion::Peaceful);
            ...
            },
            {
                "old_letter",
                "📜 旧书信",
                "在宿舍阁楼发现了一封泛黄的情书，是几十年前学长留下的。",
                "宿舍阁楼",
                [](Stats& stats, EmotionalState& emotion) {
                    emotion.addMemory("发现了一封旧情书，感受到了时光的浪漫");
            ...
            },
            {
                "campus_cat",
                "🐱 校园猫",
                "你发现了一只经常出没在食堂附近的橘猫，它似乎很喜欢你。",
                "食堂附近",
                [](Stats& stats, EmotionalState& emotion) {
                    emotion.modifyLoneliness(-10);
                    stats.modifyHappiness(15);
                    emotion.addMemory("🐱 和校园猫成为了朋友");
                }
            },
            {
                "time_capsule",
                "📦 时光胶囊",
                "在操场角落挖出了一个时光胶囊，里面装着十年前学生的梦想。",
                "操场",
                [](Stats& stats, EmotionalState& emotion) {
                    stats.modifyHappiness(10);
                    emotion.addMemory("发现了时光胶囊，感受到了时间的重量");
                }
            },
            {
                "rooftop_view",
                "🌅 天台风景",
                "教学楼天台可以看到整个校园的风景，日落时分格外美丽。",
                "教学楼天台",
                [](Stats& stats, EmotionalState& emotion) {
                    emotion.modifyLoneliness(-20);
                    emotion.setEmotion(Emotion::Peaceful);
                    stats.modifyHappiness(15);
                }
            },
            {
                "hidden_study_room",
                "📖 秘密自习室",
                "在图书馆最深处发现了一间安静的自习室，几乎没人知道。",
                "图书馆",
                [](Stats& stats, EmotionalState& emotion) {
                    stats.modifyHappiness(5);
                    emotion.addMemory("找到了一个秘密自习室，学习效率提升");
    ...
    bool discover(const std::string& id, Stats& stats, EmotionalState& emotion) {
        for (auto& d : discoveries) {
            if (d.id == id && !d.found) {
                d.found = true;
                foundIds.push_back(id);
                d.effect(stats, emotion);
                std::cout << "\n🔍 发现: " << d.name << "\n";
    ...
    bool discoverRandom(Stats& stats, EmotionalState& emotion) {
        std::vector<Discovery*> unfound;
        for (auto& d : discoveries) {
            if (!d.found) unfound.push_back(&d);
        }
        if (unfound.empty()) return false;
        
        std::uniform_int_distribution<int> dist(0, unfound.size() - 1);
        std::mt19937 rng(std::time(nullptr));
        auto& d = unfound[dist(rng)];
        return discover(d->id, stats, emotion);
    }

    std::vector<Discovery> getDiscoveries() const { return discoveries; }
    int getFoundCount() const { return foundIds.size(); }
    int getTotalCount() const { return discoveries.size(); }
        ...
        events = {
            ...
            },
            {
                ...
                [](Stats& stats, CourseManager& cm, SkillManager& sm) {
                    ...
                    stats.getEmotion().modifyLoneliness(20);
                    stats.incrementEventCount();
                },
                4
            },
...
private:
    ...
    std::vector<std::string> specialDialogues;  // v3.2 新增
...
public:
    ...
    void interact(Stats& stats, CourseManager& cm) {
        ...
        if (rel) {
            ...
            if (!rel->getIsRomantic() && favorability >= 50) {
                rel->confess();
            }
    ...
    void initAchievements() {
        achievements = {
            ...
            {"heart_breaker", "💔 心碎", "经历一次分手", "💔",
             [](const json& data) { 
                 for (auto& rel : data["relationships"]) {
                     if (rel["status"].get<int>() == 7) return true; // Broken
                 }
                 return false;
             }},
    ...
    void initStory() {
        storyEvents = {
            ...
            {2, "📝 期中考试", 
             ...
             "你怎么准备？",
             {
                 {"📖 泡图书馆通宵", [](json& data, Stats& stats) { 
                     data["story_choices"]["midterm"] = "hard";
                     stats.modifyEnergy(-20);
                     stats.getEmotion().setEmotion(Emotion::Anxious);
                     data["midterm_bonus"] = 15;
                     stats.addBuff("学习效率+20%", 2);
                 }},
                 {"🤝 和同学组队复习", [](json& data, Stats& stats) { 
                     data["story_choices"]["midterm"] = "group";
                     stats.modifyHappiness(10);
                     stats.getEmotion().modifyLoneliness(-15);
                     data["midterm_bonus"] = 10;
                 }},
                 {"😴 正常复习，保持睡眠", [](json& data, Stats& stats) { 
                     data["story_choices"]["midterm"] = "balance";
                     stats.getEmotion().setEmotion(Emotion::Peaceful);
            ...
            {3, "🏆 社团招新", 
             ...
             "你选择加入哪个社团？",
             {
                 {"💻 编程社", [](json& data, Stats& stats) { 
                     data["story_choices"]["club"] = "programming";
                     stats.getSkills().push_back("编程能力");
                     stats.modifyHappiness(10);
                 }},
                 {"⚽ 体育社", [](json& data, Stats& stats) { 
                     data["story_choices"]["club"] = "sports";
                     stats.modifyEnergy(10);
                     stats.getEmotion().modifyLoneliness(-10);
                 }},
                 {"🎨 文艺社", [](json& data, Stats& stats) { 
            ...
            {5, "🎓 考研还是就业", 
             ...
             "你选择哪条路？",
             {
                 {"📚 考研深造", [](json& data, Stats& stats) { 
                     data["story_choices"]["graduate"] = "master";
                     data["grad_success"] = true;
                     stats.getEmotion().setEmotion(Emotion::Excited);
                 }},
                 {"💼 直接就业", [](json& data, Stats& stats) { 
                     data["story_choices"]["graduate"] = "job";
                     stats.modifyMoney(5000);
                     stats.getEmotion().setEmotion(Emotion::Happy);
                 }},
                 {"🌍 出国留学", [](json& data, Stats& stats) { 
                     data["story_choices"]["graduate"] = "abroad";
                     data["grad_success"] = true;
                     stats.modifyMoney(-2000);
                     stats.getEmotion().setEmotion(Emotion::Excited);
                 }}
             },
             "graduation"
            ...
            {3, "💕 心动瞬间", 
             ...
             "你要怎么做？",
             {
                 {"💌 主动表白", [](json& data, Stats& stats) { 
                     data["story_choices"]["love"] = "confess";
                     auto* rel = stats.getRelationship("学姐");
                     if (rel) rel->confess();
                     stats.getEmotion().setEmotion(Emotion::Loved);
                 }},
                 {"🤝 保持朋友关系", [](json& data, Stats& stats) { 
                     data["story_choices"]["love"] = "friend";
                     stats.modifyHappiness(5);
                 }},
                 {"😊 顺其自然", [](json& data, Stats& stats) { 
                     data["story_choices"]["love"] = "wait";
                     stats.getEmotion().setEmotion(Emotion::Peaceful);
                 }}
             },
             "love"
...
// ============================================================
// 12. Interactable - 建筑交互 (v3.2 增加探索)
// ============================================================
class Interactable {
public:
    ...
    virtual void interact(Stats& stats, CourseManager& cm, ExplorationSystem& exploration) = 0;
...
public:
    ...
    void interact(Stats& stats, CourseManager& cm, ExplorationSystem& exploration) override {
        ...
        // 随机发现
        if (std::rand() % 100 < 15) {
            exploration.discover("old_letter", stats, stats.getEmotion());
        }
    }
};
...
public:
    ...
    void interact(Stats& stats, CourseManager& cm, ExplorationSystem& exploration) override {
        ...
        if (!courses.empty()) {
            ...
            // 随机发现
            if (std::rand() % 100 < 20) {
                exploration.discover("secret_garden", stats, stats.getEmotion());
            }
            if (std::rand() % 100 < 10) {
                exploration.discover("hidden_study_room", stats, stats.getEmotion());
            }
        } else {
            std::cout << "⚠️ 请先选修课程！\n";
        }
...
public:
    ...
    void interact(Stats& stats, CourseManager& cm, ExplorationSystem& exploration) override {
        ...
        // 随机发现
        if (std::rand() % 100 < 10) {
            exploration.discover("campus_cat", stats, stats.getEmotion());
        }
    }
};
...
public:
    ...
    bool save(const Stats& stats, const CourseManager& cm, 
              ...
              const ExplorationSystem& exploration) {
        json data = {
            {"stats", stats.toJson()},
            {"courses", cm.toJson()},
            {"skills", sm.toJson()},
    ...
    bool load(Stats& stats, CourseManager& cm, 
        ...
        if (data.contains("exploration")) exploration.fromJson(data["exploration"]);
...
private:
    ...
    sf::RectangleShape emotionPanel;

public:
    GameManager() {
        window.create(sf::VideoMode(1024, 768), "🎓 学生生活模拟器 v3.2", sf::Style::Close);
        ...
        messages.push_back("🎓 学生生活模拟器 v3.2");
        messages.push_back("💕 新功能: 恋爱系统 | 探索系统 | 情感状态 | 故事分支");
    ...
    void setupUI() {
        ...
        emotionPanel.setSize({700, 40});
        emotionPanel.setPosition({320, 560});
        emotionPanel.setFillColor({0, 0, 0, 0});
    }
    ...
    void checkBuildingInteraction() {
        ...
        for (auto& building : buildings) {
            ...
            if (bounds.contains(pos.x, pos.y)) {
                std::cout << "\n🏛️ " << building->getEmoji() << " " 
                          << building->getName() << "\n";
                building->interact(stats, courseManager, explorationSystem);
                ...
                // 触发探索
                if (std::rand() % 100 < 10) {
                    explorationSystem.discoverRandom(stats, stats.getEmotion());
                }
    ...
    void checkAchievements() {
        json data = {
            ...
            {"discoveries", explorationSystem.getFoundCount()},
            {"discoveries_total", explorationSystem.getTotalCount()},
            {"graduated", false}
        };
        ...
        switch (option) {
            ...
            case 9: {
                if (stats.getSemester() >= 8 && 
                    ...
                    addMessage("💕 恋爱关系: " + std::to_string(std::count_if(stats.getRelationships().begin(),
                        stats.getRelationships().end(), [](const Relationship& r) { return r.getIsRomantic(); })) + "段");
                    addMessage("🔍 发现秘密: " + std::to_string(explorationSystem.getFoundCount()) + "个");
                    isRunning = false;
                } else {
                    std::string reason;
                    if (stats.getSemester() < 8) 
    ...
    void render() {
        ...
        sf::Text hint;
        hint.setFont(font);
        hint.setString("方向键 | Enter交互 | 1-9菜单 | S存档 | L读档 | R改名 | E事件 | A成就 | F恋情");
    ...
    void drawStatusInfo() {
        ...
        ss << "📊 " << stats.getName() << "\n"
           ...
           << "/" << achievementManager.getTotalCount() << "\n"
           << "🔍 发现: " << explorationSystem.getFoundCount() 
           << "/" << explorationSystem.getTotalCount() << "\n"
           << "💕 恋情: " << std::count_if(stats.getRelationships().begin(),
               stats.getRelationships().end(), [](const Relationship& r) { return r.getIsRomantic(); }) << "段";
        ...
        emotionText.setCharacterSize(13);
        emotionText.setFillColor(sf::Color::Cyan);
        emotionText.setPosition(20, 265);
    ...
    void drawMenu() {
        ...
        for (size_t i = 0; i < items.size(); i++) {
            ...
            text.setPosition(20, 280 + i * 35);
            window.draw(text);
        }
        
        sf::Text hint;
        hint.setFont(font);
        hint.setString("🎲 E=事件  🏆 A=成就  💕 F=恋情");
        hint.setCharacterSize(13);
        hint.setFillColor(sf::Color::Yellow);
        hint.setPosition(20, 600);
        window.draw(hint);
    }
...
int main() {
    ...
    std::cout << "🎓 学生生活模拟器 v3.2\n";
    std::cout << "========================================\n";
    std::cout << "✨ v3.2 新功能:\n";
    std::cout << "  💕 恋爱系统 (表白/分手/恋爱关系)\n";
    std::cout << "  🔍 探索系统 (发现校园秘密)\n";
    std::cout << "  🎭 情感状态 (心情/孤独感/稳定性)\n";
    std::cout << "  📖 故事分支 (多种结局)\n";
    std::cout << "  🏆 新增成就 (初恋/心碎/探索者等)\n";
    std::cout << "========================================\n\n";
